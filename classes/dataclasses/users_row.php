...
class Users_Row extends row{
    ...
    function ValidateDelete( $database ){
         ...
         if( 0 != $database->countOf("GroupList","UID = $this->userid")){
            throw new validateException("A value exists in the GroupList table.");
         }

         if( 0 != $database->countOf("UserTeamList","teamlist_userid = $this->userid")){
            throw new validateException("A value exists in the UserTeamList table.");
         }
     }
    ...
    function login( $database ){
      $sql = "SELECT * FROM Users LEFT JOIN ContactInfo on contactinfo_id = id  WHERE username =:username AND password =:password";
      $database->prepare($sql, array(":username" -> $this->username, ":password" -> $this->password));
   ...
   static function Authentication($db){
      if ( false == session_start() ) {
        TemplateLogger::Debug("Session start failed.");
      }
   ...
   function hasEditContactPermisions( $Team_ID ){
      $editContact = flase;
      if( $this->hasPermisions( Groups_Row::Administrators )){
         $editContact = true;
      }
      else{
         while ( $team = $this->fetchNextTeamObject()){
   ...
   function getUserNewForm($sqlEdit = null){
      ...
      $tpl->set('PhoneWork',$sqlEdit != NULL ? $sqlEdit->_contactInfoObject->PhoneWork : NULL);
      $tpl->set('PhoneCell',$sqlEdit != NULL ? $sqlEdit->_contactInfoObject->PhoneCell : NULL);
      $tpl->set('Email',$sqlEdit != NULL ? $sqlEdit->_contactInfoObject->Email : NULL);
      $tpl->set('username',$sqlEdit != NULL ? $sqlEdit->username : NULL);
      $tpl->set('userid',$sqlEdit != NULL ? $sqlEdit->userid : NULL);
      $tpl->set('Id',$sqlEdit != NULL ? $sqlEdit->contactinfo_id : NULL);
      $tpl->set('password',$sqlEdit != NULL ? $sqlEdit->password : NULL);
      $tpl->set('Edit',$sqlEdit != null ? "TRUE" : "FALSE");
