...
function DisplayStats($teams, $sqlExecutorTeams, $sqlPoints, $navbar){   
   ...
   $tpl->set('stats', $sqlPoints->fetchThisTemplate("../templates/Stats.tpl.php"));
   $teams->set('content', $tpl->fetch('TeamStats.tpl.php'));
   echo $teams->fetch('../templates/pages/teams.tpl.php');    
}
...
function DisplaySchedule($teams, $sqlExecutorTeams, $sqlExecutorSchedule, $navbar){ 
   ...
   $teams->set('content', $sqlExecutorSchedule->fetchThisTemplate("../templates/TeamSchedule.tpl.php",array( Team_ID => $team->Team_ID, 
...
try{
    ...
    $teams->set('sidebar',$sql_Executor_Teams->fetchThisTemplate('../templates/TeamSideBar.tpl.php', array(navigation => $navigation, clubResults => $sql_Executor_TeamsClub))); 
}
catch(Exception $e){
    $teams->error($e);
}
